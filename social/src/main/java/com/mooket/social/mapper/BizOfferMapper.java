...
public interface BizOfferMapper extends BaseMapper<BizOffer> {
    ...
    @Select({"<script>SELECT * FROM biz_offer WHERE status = 'ACTIVE' AND data_date >= CURRENT_DATE - INTERVAL '1 day' AND country = #{country} AND REPLACE(factory_no, ' ', '') = REPLACE(#{factoryNo}, ' ', '') <if test='productName != null and productName.length() > 0'> AND product_name = #{productName} </if> <if test='category != null'> AND category = #{category} </if><if test='offerType != null'> AND offer_type = #{offerType} </if> ORDER BY publish_time DESC</script>"})
    ...
    @Select({"<script>SELECT COUNT(DISTINCT COALESCE(merchant_id, -1)) FROM biz_offer WHERE status = 'ACTIVE' AND data_date &gt;= CURRENT_DATE - INTERVAL '1 day' AND country = #{country} AND REPLACE(factory_no, ' ', '') = REPLACE(#{factoryNo}, ' ', '') <if test='productName != null and productName.length() > 0'> AND product_name = #{productName} </if> <if test='category != null'> AND category = #{category} </if><if test='offerType != null'> AND offer_type = #{offerType} </if></script>"})
    ...
    @Select({"<script>SELECT * FROM biz_offer WHERE status = 'ACTIVE' AND data_date &gt;= CURRENT_DATE - INTERVAL '1 day' AND country = #{country} AND REPLACE(factory_no, ' ', '') = REPLACE(#{factoryNo}, ' ', '') <if test='productName != null and productName.length() > 0'> AND product_name = #{productName} </if> <if test='category != null'> AND category = #{category} </if><if test='offerType != null'> AND offer_type = #{offerType} </if> <if test='merchantIds != null and merchantIds.size() > 0'>AND merchant_id IN <foreach collection='merchantIds' item='merchantId' open='(' separator=',' close=')'>#{merchantId}</foreach></if> ORDER BY publish_time DESC</script>"})
    ...
    @Select({"<script>SELECT * FROM biz_offer WHERE status = 'ACTIVE' AND data_date &gt;= CURRENT_DATE - INTERVAL '1 day' AND country = #{country} AND REPLACE(factory_no, ' ', '') = REPLACE(#{factoryNo}, ' ', '') <if test='productName != null and productName.length() > 0'> AND product_name = #{productName} </if> <if test='category != null'> AND category = #{category} </if><if test='offerType != null'> AND offer_type = #{offerType} </if> AND merchant_id IS NULL ORDER BY publish_time DESC</script>"})
