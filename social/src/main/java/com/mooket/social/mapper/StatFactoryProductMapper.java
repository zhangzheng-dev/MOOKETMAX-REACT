...
public interface StatFactoryProductMapper extends BaseMapper<StatFactoryProduct> {
    ...
    @Select({"SELECT factory_id, product_id, category, avg_price FROM stat_factory_product WHERE stat_date = #{statDate}"})
    @Results({@Result(property = "factoryId", column = "factory_id"), @Result(property = "productId", column = "product_id"), @Result(property = "category", column = "category"), @Result(property = "avgPrice", column = "avg_price")})
    List<StatFactoryProduct> findByDate(@Param("statDate") LocalDate statDate);
}
