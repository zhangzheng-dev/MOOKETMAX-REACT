...
public interface StatCountryProductMapper extends BaseMapper<StatCountryProduct> {
    ...
    @Select({"SELECT country, product_id, category, avg_price FROM stat_country_product WHERE stat_date = #{statDate}"})
    @Results({@Result(property = "country", column = "country"), @Result(property = "productId", column = "product_id"), @Result(property = "category", column = "category"), @Result(property = "avgPrice", column = "avg_price")})
    List<StatCountryProduct> findByDate(@Param("statDate") LocalDate statDate);
}